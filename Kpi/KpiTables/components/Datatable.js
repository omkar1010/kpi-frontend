...
const DataTable = ({ category, subcategory,selectedNewOption }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:8080/api/getHR/${category}`);
    ...
    fetchData();
  }, [category, subcategory, selectedNewOption]);
